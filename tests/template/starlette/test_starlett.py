...
class StarletteTest(unittest.TestCase):
    def does_this_implement_getter(
        ...
        def {starlette_get_name(key_name)}(#self#)->#starlette.responses.FileResponse#:
            ...
            data = self.{key_name}
            ...
            #return starlette.responses.FileResponse(data,attachment_filename=file_name,mimetype=mime_key)#
        ...
        def validate_function():
            """def {starlette_get_name(key_name)}(#self#)->#starlette.responses.FileResponse#:"""
            self.assertEqual(
                getter.name, starlette_get_name(key_name), "Function name is wrong"
            )
            ...
            if _return:
                ...
                if isinstance(_return.value, ast.Call):
                    ...
                    if isinstance(_return.value.func, ast.Attribute):
                        ...
                        self.assertEqual(_return.value.func.attr, "FileResponse")
                        if isinstance(_return.value.func.value, ast.Attribute):
                            self.assertIsInstance(
                                _return.value.func.value.value, ast.Name
                            )
                            self.assertEqual(_return.value.func.value.attr, "responses")
                            if isinstance(_return.value.func.value.value, ast.Name):
                                self.assertEqual(
                                    _return.value.func.value.value.id, "starlette"
                                )
                            return
    ...
    def does_this_implement_setter(
        ...
        def validate_decorator():
            ...
            decorator = next(
                (
                    ...
                    and dec.attr == "setter"
                    and dec.value.id == starlette_get_name(key_name)
                ),
                None,
            )
        ...
        def validate_function_values():
            ...
            if isinstance(second_arg.annotation, ast.Attribute):
                ...
                if isinstance(second_arg.annotation.value, ast.Attribute):
                    self.assertEqual(
                        second_arg.annotation.value.attr,
                        "datastructures",
                        "Second argument annotation structure issue",
                    )
                    self.assertIsInstance(
                        second_arg.annotation.value.value,
                        ast.Name,
                        "Second argument annotation structure issue",
                    )
                    if isinstance(second_arg.annotation.value.value, ast.Name):
                        self.assertEqual(
                            second_arg.annotation.value.value.id,
                            "starlette",
                            "Second argument annotation structure issue",
                        )
                        self.assertIsInstance(setter.returns, ast.Constant)
                        if isinstance(setter.returns, ast.Constant):
                            self.assertEqual(setter.returns.value, None)
                            return
            raise Exception("")
            ...
            if assign:
                ...
                if isinstance(assign.value, ast.Attribute):
                    ...
                    if isinstance(assign.value.value, ast.Name):
                        ...
                        self.assertEqual(assign.value.attr, "content_type")
                        return
            raise Exception("")
        ...
        def validate_data_assign():
            ...
            if assign:
                self.assertIsInstance(assign.value, ast.Await)
                if isinstance(assign.value, ast.Await):
                    self.assertIsInstance(assign.value.value, ast.Call)
                    if isinstance(assign.value.value, ast.Call):
                        self.assertIsInstance(assign.value.value.func, ast.Attribute)
                        if isinstance(assign.value.value.func, ast.Attribute):
                            self.assertEqual(assign.value.value.func.attr, "read")
                            self.assertIsInstance(
                                assign.value.value.func.value, ast.Name
                            )
                            if isinstance(assign.value.value.func.value, ast.Name):
                                self.assertEqual(
                                    assign.value.value.func.value.id, "file"
                                )
                                return
            raise Exception("")
    ...
    def test_build_no_file(self):
        ...
        getter = get_property_getter(starlette_get_name(key_name), _class)
        setter = get_property_setter(starlette_get_name(key_name), _class)
    ...
    def test_build_full_change_no_diff(self):
        ...
        Starlette(key_name, key, _class).build()
        Starlette(key_name, key, _class).change(
            key_name,
            key,
        )
