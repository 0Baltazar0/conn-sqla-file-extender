...
class DynamicFileNameTest(unittest.TestCase):
    def does_this_implement_setter(
        self, key_name: str, file_name_field_name: str, setter: ast.FunctionDef
    ) -> None:
        self.assertEqual(setter.name, get_column_file_name_key(key_name))
        ...
        attribute_setter = next(
            (
                ...
                and atr.targets[0].attr == file_name_field_name
    ...
    def does_this_implement_getter(
        self, key_name: str, file_name_field_name: str, getter: ast.FunctionDef
    ):
        self.assertEqual(getter.name, get_column_file_name_key(key_name))
    ...
    def does_this_implement_field_name(
        self, key_name: str, file_name_field_name: str, _class: ast.ClassDef
    ) -> None:
        getter: ast.FunctionDef = get_property_getter(
            get_column_file_name_key(key_name), _class
        )  # type: ignore
        self.assertIsNotNone(
            getter,
            f"Getter does not exists! {key_name=},{get_column_file_name_key(key_name)} {file_name_field_name=}",
        )
        ...
        self.assertIsNotNone(
            setter,
            f"Setter does not exists! {key_name=},{get_column_file_name_key(key_name)} {file_name_field_name=}",
        )

        self.does_this_implement_getter(key_name, file_name_field_name, getter)
        self.does_this_implement_setter(key_name, file_name_field_name, setter)

    def test_simple_build_command(self) -> None:
        _class = ast.ClassDef("TestClass", [], [], [], [])
        key_name = "test_key"
        key: FileFields = {"file_name_field_name": "other_sub_key"}
        DynamicFileName(key_name, key, _class).build()
    ...
    def test_simple_rename_command_on_empty(self) -> None:
        ...
        key_name = "test_key"
        key: FileFields = {"file_name_field_name": "other_sub_key"}
        DynamicFileName("key_name", {}, _class).change(key_name, key)
        self.does_this_implement_field_name(key_name, "other_sub_key", _class)

    def test_simple_rename_command(self) -> None:
        old_key_name = "test_key"
        old_key: FileFields = {"file_name_field_name": "other_sub_key"}
        _class = ast.ClassDef(
            "TestClass",
            [],
            [],
            [
                ...
                ast.FunctionDef(
                    ...
                    [
                        ast.Attribute(
                            ast.Name(get_column_file_name_key(old_key_name)), "setter"
                        )
                    ],
                ),
            ],
        ...
        DynamicFileName(old_key_name, old_key, _class).change(new_key_name, old_key)
    ...
    def test_simple_purge_command(self) -> None:
        ...
        _class = ast.ClassDef(
            ...
            [
                ...
                ast.FunctionDef(
                    ...
                    [
                        ast.Attribute(
                            ast.Name(get_column_file_name_key(key_name)), "setter"
                        )
                    ],
                ),
            ],
        ...
        DynamicFileName(key_name, key, _class).purge()
    ...
    def test_full_history(self):
        ...
        DynamicFileName(key_name, key, _class).change(
            key_name, {"file_name_field_name": "other_sub_key_two"}
        )

        self.does_this_implement_field_name("test_key", "other_sub_key_two", _class)
        DynamicFileName(key_name, key, _class).change(
            "other_key_name", {"file_name_field_name": "other_sub_key_two"}
        )
        ...
        DynamicFileName("other_key_name", key, _class).purge()
