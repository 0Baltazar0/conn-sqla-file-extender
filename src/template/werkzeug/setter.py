...
class WerkzeugSetterTemplate:
    ...
    def build_decorator(self, _key_name: str | None = None) -> ast.Attribute:
        ...
        return ast.Attribute(ast.Name(werkzeug_get_name(key_name)), "setter")
    ...
    def rename_decorator(self, new_key_name: str) -> None:
        ...
        dec = next(
            (
                ...
                and dec.value.id == werkzeug_get_name(new_key_name)
                and dec.attr == "setter"
            ),
            None,
        )
        if not dec:
            ...
            if self._fn.decorator_list.index(dec) != len(self._fn.decorator_list) - 1:
                raise TemplateException("")
            dec.value = ast.Name(werkzeug_get_name(new_key_name))
    ...
    def rename_function_base(self, new_key_name: str) -> None:
        ...
        self._fn.name = werkzeug_get_name(new_key_name)

    def build_static_mime_definition(self) -> ast.Assign:
        "mime_type = file.mimetype"
        return ast.Assign(
            [ast.Name("mime_type")], ast.Attribute(ast.Name("file"), "mimetype")
        )
    ...
    def build_data_assign(self) -> ast.Assign:
        ...
        return ast.Assign(
            [ast.Name("data")],
            ast.Call(ast.Attribute(ast.Name("file"), "read"), [], []),
        )
